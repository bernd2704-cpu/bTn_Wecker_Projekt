...
// Basis: bTn_Alarm_2v0 – FreeRTOS + State Machine + Stabilitätsverbesserungen
...
String PGMInfo = "bTn_Alarm_2v0.ino";
    ...
    switch (tsState) {
      ...
      case TS_PRESSED:
        if (!padPressed[activeIdx]) {                                                    // losgelassen vor HOLD → kein weiterer EVT
          ...
          xQueueSend(inputQueue, &evt, 0);                                               // EVT bei HOLD-Eintritt
          lastRepeat = now;
          tsState    = TS_REPEAT;
        }
        break;
      ...
      case TS_REPEAT:
        if (!padPressed[activeIdx]) {                                                    // losgelassen → zurück zu IDLE
          ...
          uint8_t evt = EVT_ID[activeIdx];
          xQueueSend(inputQueue, &evt, 0);                                               // Wiederholungs-EVT
          lastRepeat = now;
        }
        break;
    }
