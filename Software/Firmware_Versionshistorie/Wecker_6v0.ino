...
// Basis: bTn_Alarm_6v0 – FreeRTOS + State Machine + WiFi-Konfigurator
...
const char PGMInfo[] = "bTn_Alarm_6v0";                                                  // PROGMEM-fähig; kein String-Heap-Fragment
...
void showTime() {
  time(&now);
  localtime_r(&now, &tm);
  snprintf(datum, sizeof(datum), "%02u.%02u.%04u", tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900);
  snprintf(zeit,  sizeof(zeit),  "%02u:%02u:%02u",  tm.tm_hour, tm.tm_min,    tm.tm_sec);
  t_sec  = tm.tm_sec;
  t_min  = tm.tm_min;
  t_hour = tm.tm_hour;
}
...
void zeigeZ10R(int xPos, int yPos, String TXT) {
...
void menu(int page) {
  switch (page) {
    ...
    case 6:
      display.clear();
      zeigeZ10C(64,  0, "Kuckuck-Aktiv");
      zeigeZ16C(5,  17, "\x3E");
      zeigeZ16C(15, 17, "\x3E");
      zeigeZ16L(50, 17, "von");
      zeigeZ16C(105,17, str_cot);
      zeigeZ16C(5,  34, "\x3E");
      zeigeZ16C(15, 34, "\x3E");
      zeigeZ16L(50, 34, "bis");
      zeigeZ16C(105,34, str_coff);
      break;
    case 7:
      display.clear();
  ...
  switch (cuckooState) {
    ...
    case CUCKOO_IDLE:
      if (t_min == 0 && t_sec == 0 && cuckoo_on && t_min != lastCuckooMin) {
        ...
        bool inTimeWindow   = (t_hour >= cuckoo_onTime && t_hour <= cuckoo_offTime);
