...
// Basis: bTn_Alarm_7v6 – FreeRTOS + State Machine + WiFi-Konfigurator
...
const char PGMInfo[] = "bTn_Alarm_7v6";                                                  // PROGMEM-fähig; kein String-Heap-Fragment
...
void readNVR() {
  ...
  if (sound1_play < 1) sound1_play = 1;                // DFPlayer: Dateinummer min. 1
  if (sound2_play < 1) sound2_play = 1;                // Obergrenze erst nach mp3Count bekannt
...
static UiState uiDispatch(UiState s, uint8_t evt) {
  ...
  if (evt == EVT_S3) {
    ...
    if (t_now - lastBtnMs[2] >= BTN_DEBOUNCE_MS) {
      lastBtnMs[2] = t_now;
  ...
  switch (alarmState) {
    ...
    case ALARM_RUNNING:
      if (delayFunction(t_start6, ALARM_POLL_MS)) {
        ...
        if (xSemaphoreTake(playerMutex, pdMS_TO_TICKS(200)) == pdTRUE) {
          st = player.readState();
          vTaskDelay(pdMS_TO_TICKS(1));                                                   // kurze Pause: DFPlayer-Antwort stabilisieren
...
static void inputTask(void *pvParam) {
  ...
  while (true) {
    ...
    if (evt == EVT_S1) {
      uint32_t t_now = millis();
      if (t_now - lastBtnMs[0] >= BTN_DEBOUNCE_MS) {
        lastBtnMs[0] = t_now;
    ...
    if (evt == EVT_S2) {
      uint32_t t_now = millis();
      if (t_now - lastBtnMs[1] >= BTN_DEBOUNCE_MS) {
        lastBtnMs[1] = t_now;
        S2_SW = !S2_SW;
        if (S2_SW) {
          if (wheel_on) { digitalWrite(E2, HIGH); }
