...
// Basis: bTn_Alarm_9v2 – FreeRTOS + State Machine + WiFi-Konfigurator
...
const char PGMInfo[] = "bTn_Alarm_9v2";                                                  // PROGMEM-fähig; kein String-Heap-Fragment
...
// ── Konfiguration ────────────────────────────────────────────
#include "SysConf_9v2.h"           // Pin-Belegung, Timing-Konstanten, Touch-Schwellwerte
#include "WEB.h"
...
static void touchTask(void *pvParam) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    webLogf("Touch Pad %d  Baseline: %u  Threshold: %u",
                  i, baseline[i],
  ...
  while (true) {
    ...
    if (tsState == TS_IDLE && millis() - lastRecal >= TOUCH_RECAL_MS) {
      ...
      webLog("Touch Baseline rekalibriert:");
      for (int i = 0; i < 4; i++) {
        webLogf("  Pad %d  Baseline: %u  Threshold: %u",
                      i, baseline[i],
                      (baseline[i] > TOUCH_DROP) ? baseline[i] - TOUCH_DROP : baseline[i] - baseline[i] / 5);
      }
      lastRecal = millis();
...
static void stackMonTask(void *pvParam) {
  while (true) {
    ...
    webLog("── Stack High-Water Marks (Words frei) ──────────");
    webLogf("  touchTask   : %4u", uxTaskGetStackHighWaterMark(hTouchTask));
    webLogf("  wifiTask    : %4u", uxTaskGetStackHighWaterMark(hWifiTask));
    webLogf("  nvrTask     : %4u", uxTaskGetStackHighWaterMark(hNvrTask));
    webLogf("  inputTask   : %4u", uxTaskGetStackHighWaterMark(hInputTask));
    webLogf("  displayTask : %4u", uxTaskGetStackHighWaterMark(hDisplayTask));
    webLogf("  alarmTask   : %4u", uxTaskGetStackHighWaterMark(hAlarmTask));
    webLogf("  watchdogTask: %4u", uxTaskGetStackHighWaterMark(hWatchdogTask));
    webLogf("  stackMonTask: %4u", uxTaskGetStackHighWaterMark(nullptr));
    webLogf("  Freier Heap : %u Bytes", esp_get_free_heap_size());
    webLog("──────────────────────────────────────────");
  }
}
...
static void webLogTask(void *pvParam) {
  ...
  logServer.on("/", HTTP_GET, [&logServer]() {
    ...
    String html =
      ...
      "h2{color:#BDD7EE;margin-bottom:8px}h3{color:#888;font-weight:normal;font-size:13px;margin:0 0 12px}"
      "#log{background:#0d0d1a;border:1px solid #333;border-radius:6px;padding:12px;"
      "white-space:pre-wrap;word-break:break-all;max-height:80vh;overflow-y:auto;font-size:13px}"
      ".ts{color:#4A9EFF}.ok{color:#6BCB77}.err{color:#FF6B6B}.warn{color:#FFD93D}"
      "</style></head><body>"
      "<h2>&#x1F553; bTn Wecker 9v2 &ndash; Web-Log</h2>"
      "<h3>IP: " + ip + ":" + String(WEBLOG_PORT) + " &nbsp;|&nbsp; Auto-Refresh: 10 s"
      " &nbsp;|&nbsp; Aktualisiert: <span id='upd'></span></h3>"
      "<div id='log'>";

    // Ring-Puffer lesen (älteste zuerst für chronologische Reihenfolge)
  ...
  logServer.begin();
  webLogReady = true;
  webLogf("[webLog] HTTP-Server gestartet auf http://%s:%u",
          WiFi.localIP().toString().c_str(), (unsigned)WEBLOG_PORT);
...
void setup() {
  ...
  const esp_task_wdt_config_t twdt_cfg = {
    .timeout_ms    = WDT_HARDWARE_MS,  // aus SysConf_9v2.h
